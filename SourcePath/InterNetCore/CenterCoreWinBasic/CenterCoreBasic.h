...
class CenterCoreBasic : public ICenterCore
{
public:
	CenterCoreBasic();
	virtual ~CenterCoreBasic();

#pragma region Parent Interface
	//	初始化
	virtual bool Initialize(ICenterCore* pCoreRoot) override;
	...
	//	获取核心根
	virtual ICenterCore* GetCoreRoot() override;

	//	注册
	virtual bool RegisterServerCore(ICenterCore* pCore) override;
...
#pragma region variable
	ICenterCore*							m_pParentCore;				//	上级节点
	char									m_strName[64];				//	节点名称
	std::map<std::string, ICenterCore*>		m_dicCoreElement;			//	子节点
